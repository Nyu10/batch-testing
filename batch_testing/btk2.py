...
def sens_burns(n, k, sp_i, sens_i, alpha, beta):
    if k > 0:
        return 1-sp_i + (sens_i + sp_i - 1)*(k/n) ** (alpha) + beta * k
    else:
        return sens_i


def sens_hwang(n,k, d, p, sens_i):
    if k > 0:
        return p/(1 - (1 - p) ** (d * n))
    else:
        return sens_i

#kwargs = {'sens_i': 0.99, 'alpha': 0.01, 'beta': 0.22, 'sp': 0.99}
#print(sens_burns(n = 10, k = 1, **kwargs))
#print(obj(sens_burns, n = 10, **kwargs))
...
kwargs = {'sens_i': 0.99, 'alpha': 0.01, 'beta': 0.03, 'sp_i': 0.99}
print(obj(sens_burns, n = 10, p = 0.1, sp = 0.99, **kwargs))

kweys = {'sens_i': 0.95, 'alpha': 0.33, 'beta':0.11, 'sens_func': sens_burns, 'sp_i': 0.99}
print(linear_scan(10, p = 0.05, sp = 0.95,**kweys))
...
prob_threshold = 1, seq = True, batch_limit = 32):
    ...
    neg_array[:,1] = 0
    individual_test, individual_con = conventional_test(pos_array, 1-sens_i, 1-sp, repeat, seq)
